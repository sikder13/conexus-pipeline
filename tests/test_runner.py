...
from lib.nodes import NODE_REGISTRY, Node, NodeResult, RunContext, register
from lib.runner import CycleError, deep_merge, merge_notes, run_nodes, topological_order
...
class FakeDB:
    ...
    def list_work_items(self, node_name, statuses, limit=None):
        rows = [
            dict(i)
            for i in self.items
            if i["node_name"] == node_name and i["status"] in statuses
        ]
    ...
    def list_work_items_for_prospects(self, prospect_ids, node_names):
        return [
            {"prospect_id": i["prospect_id"], "node_name": i["node_name"], "status": i["status"]}
            for i in self.items
